...
load("data/offenses_known_yearly_1960_2019.rda")
ucr <- offenses_known_yearly_1960_2019
rm(offenses_known_yearly_1960_2019)
gc()
```
...
```{r agenciesReportingMap, fig.cap = "The number of agencies in each state that reported at least one crime in 2019 to NIBRS."}
admin_map <-
  administrative %>%
  distinct(ori, .keep_all = TRUE) %>%
  count(state)

missing_states <- data.frame(state = unique(fifty_states$id[!fifty_states$id %in% admin_map$state]),
                             n = 0)

admin_map <- bind_rows(admin_map, missing_states)
admin_map %>%
  ...
  scale_fill_gradient(low = "white", high = "red", breaks = c(0, 100, 200, 300, 400, 500, 619)) 
```
...
```{r agenciesReportingMapPercent, fig.cap = "Agencies in each state reporting at least one crime to NIBRS in 2019 as a percent of agencies that reported UCR Offenses Known and Clearances by Arrests data in 2019."}
...
admin_map$percent <- admin_map$percent * 100
...
```{r agenciesReportingTable, fig.cap = "The number of agencies in each state reporting to NIBRS and to UCR in 2019. Also shows NIBRS reporting in each state as a percent of UCR reporting."}
